...
public class playerController : MonoBehaviour
{
    ...
    public GameObject Bulletcylinder;
    ...
    {//탄막은 z 로 발사, x로 총알(붐)을 발사할 수 있으며 총알은 장탄수 제한이 있으며 다 떨어졌을 시 c키를 눌러 재장전을 해야함
        ...
        if (Input.GetKeyDown(KeyCode.X))//총알 발사 
        {
            if (Bulletcylinder != null)
            {
                Bulletcylinder.GetComponent<PlayerBulleCount>().bulletfire();
            }
        }
        ...
        {//0.5초정도 딜레이
            if (Bulletcylinder != null)
            {
                Bulletcylinder.GetComponent<PlayerBulleCount>().rerode();
            }
        }
